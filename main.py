...
def api_delete_game():
    ### FIX: Add robust check to prevent calling .get() on None.
...
def api_load_game():
    ...
    story_title = data.get("story_title")
    if not story_title: return jsonify({"error": "Story title required."}), 400
    game_state = load_game_local(get_safe_filename(story_title))
    if game_state:
...
def start_new_game():
    ### FIX: Add robust check for safety and consistency.
    if not request.is_json:
        return jsonify({"error": "Invalid request: body must be JSON."}), 400
    data = request.get_json()
    ...
    unseen_engine_updates = {"events": "No off-screen events to report."}
...
def send_message():
    ...
    if "Error:" not in ai_response:
        ...
        conversation_history.pop() # Remove user's input if AI fails
        return jsonify({"error": ai_response}), 500
